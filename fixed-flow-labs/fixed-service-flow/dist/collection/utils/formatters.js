 ...
 */
export const formatDateSpanish = (value) => {
    const date = typeof value === 'string' ? new Date(value) : value;
    if (isNaN(date.getTime()))
        return '';
 ...
 */
export const formatAddress = (street, city, zipCode, country = 'Puerto Rico') => {
 ...
 * Formats a full name
 */
export const formatFullName = (firstName, secondName, lastName, secondLastName) => {
 ...
 */
export const formatContractDuration = (months) => {
    if (months === 0)
 ...
 */
export const formatContractCosts = (installation, activation, modem) => {
    const costs = [];
    if (installation > 0)
