...
class CoverageService {
    ...
    async validateCoverage(latitude, longitude) {
        ...
        const response = await token_service.httpService.post('api/Catalogue/getInternetPlans', {
...
class PlansService {
    ...
    async getPlans(serviceType, catalogId = 0) {
        ...
        const response = await token_service.httpService.post('api/Plans/getPlansInternet', {
            catalogID: catalogId,
            type: apiType,
        });
    ...
    async addToCart(plan, parentCartId = 0, parentProductId = 0, installments = 0, creditClass = '') {
        ...
        const response = await token_service.httpService.post('api/Card/addToCart', {
    ...
    async addToCartCurrentPlan(productId, cartId) {
        ...
        return await token_service.httpService.postFormData('api/Plans/addToCartCurrentPlan', formData);
    }
    // ------------------------------------------
    ...
    async deleteFromCart(cartId, productId = 0) {
        await token_service.tokenService.ensureToken();
        const response = await token_service.httpService.post('api/Card/deleteItem', {
            cartId: cartId,
            productId: productId,
        });
        if (!response.hasError) {
...
class RequestService {
    ...
    async submitRequest(payload) {
        ...
        const response = await token_service.httpService.postFormData('api/Orders/internetServiceRequest', formData);
        return response;
    }
    // ------------------------------------------
    // GET ORDER DETAILS
    ...
    async getOrder(orderId) {
        await token_service.tokenService.ensureToken();
        const response = await token_service.httpService.get(`api/Orders/getOrder?orderId=${encodeURIComponent(orderId)}`);
        return response;
    }
    // ------------------------------------------
    ...
    async sendConfirmation(orderId, email) {
        ...
        const response = await token_service.httpService.postFormData('api/Orders/sendConfirmation', formData);
        return response;
    }
    // ------------------------------------------
    // BUILD PAYLOAD
...
const FixedServiceFlow = class {
    ...
    handleApiUrlChange(newValue) {
        if (newValue) {
            token_service.httpService.setBaseUrl(newValue);
        }
    }
    // ------------------------------------------
    // LIFECYCLE
    ...
    async componentWillLoad() {
        ...
        if (this.apiUrl) {
            token_service.httpService.setBaseUrl(this.apiUrl);
        }
        // Set initial step
        if (this.initialStep) {
            this.currentStep = this.initialStep;
            token_service.flowActions.setStep(this.initialStep);
        }
        // Initialize token
    ...
    async initializeToken() {
        ...
        this.error = null;
        try {
            const success = await token_service.tokenService.initialize();
            if (!success) {
    ...
    handleStepChange = (direction) => {
        ...
        if (direction === 'forward') {
            ...
            token_service.flowActions.nextStep();
        }
        else {
            to = Math.max(this.currentStep - 1, 1);
            token_service.flowActions.prevStep();
        }
        this.currentStep = to;
        token_service.flowActions.setStep(to);
    ...
    _goToStep = (step) => {
        ...
        token_service.flowActions.setStep(step);
    ...
    handleFlowCancel = () => {
        ...
        token_service.flowActions.resetFlow();
        this.currentStep = 1;
        token_service.flowActions.setStep(1);
     ...
     */
    isClaroHogar() {
        const serviceType = token_service.state.location?.serviceType?.toUpperCase();
...
const StepCatalogue = class {
    ...
    async loadProducts() {
        ...
        try {
            ...
            const subcatalogId = product_service.catalogueService.FILTER_INTERNET_INALAMBRICO;
            const response = await product_service.catalogueService.listCatalogue(subcatalogId, 1, '');
    ...
    async loadProductsDetails(products) {
        ...
        const detailPromises = productsToLoad.map(async (product) => {
            ...
            try {
                const response = await product_service.productService.getEquipmentDetail(product.productId);
    ...
    handleSelectProduct = async (product) => {
        ...
        try {
            ...
            const subcatalogId = parseInt(product_service.catalogueService.FILTER_INTERNET_INALAMBRICO, 10);
            product_service.catalogueService.storeProductInSession(product);
            product_service.productService.storeSelectedProduct(product, subcatalogId);
            // Add to cart
            console.log('[StepCatalogue] Adding to cart:', product.productName);
            const cartResponse = await cart_service.cartService.addToCart(productDetail, product.installments || 24, 1);
    ...
    renderUnavailableModal() {
        ...
        return (index.h("div", { class: "unavailable-modal-overlay", onClick: this.handleCloseUnavailableModal }, index.h("div", { class: "unavailable-modal", onClick: (e) => e.stopPropagation() }, index.h("div", { class: "unavailable-modal__icon" }, index.h("svg", { viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", "stroke-width": "2" }, index.h("circle", { cx: "12", cy: "12", r: "10" }), index.h("line", { x1: "12", y1: "8", x2: "12", y2: "12" }), index.h("line", { x1: "12", y1: "16", x2: "12.01", y2: "16" }))), index.h("h3", { class: "unavailable-modal__title" }, "Producto no disponible"), index.h("p", { class: "unavailable-modal__message" }, "Lo sentimos, este producto no est\u00E1 disponible en este momento. Por favor, seleccione otro equipo."), index.h("button", { class: "unavailable-modal__button", onClick: this.handleCloseUnavailableModal }, "Entendido"))));
    }
    /**
    ...
    renderProductCard(product) {
        ...
        return (index.h("div", { class: {
            ...
            }, onClick: () => !this.isAddingToCart && this.handleSelectProduct(product) }, index.h("div", { class: "product-card__content" }, index.h("div", { class: "product-card__image" }, index.h("img", { src: product.imgUrl, alt: product.productName, loading: "lazy" })), index.h("div", { class: "product-card__info" }, index.h("h3", { class: "product-card__name" }, product.productName), product.installments > 0 && (index.h("div", { class: "product-card__financed" }, index.h("span", { class: "product-card__financed-label" }, "Financiado"), index.h("span", { class: "product-card__financed-price" }, formatPrice(product.update_price || 0), "/mes"), index.h("span", { class: "product-card__installments" }, product.installments, " plazos"))), index.h("p", { class: "product-card__regular-price" }, "Precio regular: ", formatPrice(product.regular_price || 0)))), index.h("div", { class: "product-card__footer" }, index.h("p", { class: "product-card__footer-text" }, "\u00A1Mantente conectado en todo momento!")), isProcessing && (index.h("div", { class: "product-card__processing" }, index.h("div", { class: "product-card__spinner" })))));
    }
    /**
    ...
    renderSummaryBar() {
        ...
        return (index.h("footer", { class: "step-catalogue__summary-bar" }, index.h("div", { class: "step-catalogue__summary-content" }, index.h("div", { class: "step-catalogue__summary-item" }, index.h("span", { class: "step-catalogue__summary-label" }, "PCD"), index.h("span", { class: "step-catalogue__summary-value" }, productName)), index.h("div", { class: "step-catalogue__summary-item" }, index.h("span", { class: "step-catalogue__summary-label" }, "Plan"), index.h("span", { class: "step-catalogue__summary-value" }, formatPrice(planPrice), " / mes")), index.h("div", { class: "step-catalogue__summary-item" }, index.h("span", { class: "step-catalogue__summary-label" }, "SVA"), index.h("span", { class: "step-catalogue__summary-value" }, formatPrice(svaPrice), " / mes")), index.h("div", { class: "step-catalogue__summary-item" }, index.h("span", { class: "step-catalogue__summary-label" }, "Equipo / Accesorio"), index.h("span", { class: "step-catalogue__summary-value" }, formatPrice(equipmentPrice), " / mes")), index.h("div", { class: "step-catalogue__summary-item step-catalogue__summary-item--highlight" }, index.h("span", { class: "step-catalogue__summary-label" }, "Paga hoy"), index.h("span", { class: "step-catalogue__summary-value step-catalogue__summary-value--red" }, formatPrice(payToday), " + IVU"))), index.h("button", { class: {
    ...
    // ------------------------------------------
    render() {
        return (index.h(index.Host, { key: '2b2178714bf18ddc2ba4f740f02a497f0fc48cd3' }, index.h("div", { key: '4f0911c50ecf8ca8ddc684bc929b6e060ffee845', class: "step-catalogue" }, index.h("header", { key: 'b07d68d064922ec0865b0979bdc6d5089f99a32c', class: "step-catalogue__header" }, index.h("button", { key: 'f25d525aad09fc9a035eb392a15b0daf93660933', class: "step-catalogue__back-link", onClick: this.onBack }, index.h("svg", { key: 'b70e76e40ad960548eacc463843c74c691c8f83b', viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", "stroke-width": "2" }, index.h("polyline", { key: '9dd0933e00919947e9ff88a3a357b44a9c84df21', points: "15 18 9 12 15 6" })), index.h("span", { key: 'bfbed9c9e07a1af489b32b1f879c72658ea06aa3' }, "Regresar")), index.h("h1", { key: 'b34ef77b46519546c79805005bb993d603611f1f', class: "step-catalogue__title" }, "Escoger modem para servicio fijo"), index.h("div", { key: 'efa87d6abfee8c5bdf9c9e6939cd780c9e361d9c', class: "step-catalogue__divider" })), this.isLoading && (index.h("div", { key: '0b1f7eae78137f06b40c9a21963966609228163f', class: "step-catalogue__loading" }, index.h("div", { key: '3ee83aee91e59f653df07a6cf7a6bc4c2204b8bc', class: "step-catalogue__spinner" }), index.h("p", { key: 'c2f831a70c458d6fbbc42f8c3d53e9ca31fa7639' }, "Cargando productos..."))), this.error && !this.isLoading && (index.h("div", { key: '07542ca156af5ff975beeb2442e6e8bf89d4ebeb', class: "step-catalogue__error" }, index.h("p", { key: '0900d3a0bef57fe7a15c7a44eefa9313e72b1d8a' }, this.error), index.h("button", { key: 'c4a2fd0a2f495364206eb770d6124a017246abca', onClick: () => this.loadProducts() }, "Reintentar"))), !this.isLoading && !this.error && this.products.length > 0 && (index.h("div", { key: 'daddeff0755c68c6770e39e1aeb021d1c453cd2a', class: "step-catalogue__carousel-container" }, index.h("ui-carousel", { key: '4cc8bb014d046c04722ae0d9afb36db54d4a0047', totalItems: this.products.length, gap: 24, showNavigation: false, showPagination: true, breakpoints: [
                { minWidth: 0, slidesPerView: 1 },
                { minWidth: 600, slidesPerView: 2 },
                { minWidth: 900, slidesPerView: 3 },
            ] }, this.products.map((product) => this.renderProductCard(product))))), !this.isLoading && !this.error && this.products.length === 0 && (index.h("div", { key: '1c3584c9a7aeb3604e8dfb97aebf4dd76a12db8c', class: "step-catalogue__empty" }, index.h("p", { key: 'fe9abd7e12b80df5d3eecf6ab423faca88c01664' }, "No hay productos disponibles en este momento."))), this.renderSummaryBar(), this.renderUnavailableModal())));
    }
};
...
const StepConfirmation = class {
    ...
    isCatalogueFlow() {
        ...
        const paymentResult = payment_service.paymentService.getPaymentResult();
        const hasContract = !!token_service.state.selectedContract;
        const hasFormData = !!token_service.state.formData;
    ...
    async handleCatalogueFlowConfirmation() {
        ...
        try {
            ...
            const orderBan = payment_service.paymentService.getOrderBan();
            ...
            token_service.flowActions.setOrderResult(null, this.errorMessage);
        }
    }
    // ------------------------------------------
    // METHODS
    ...
    async submitRequest() {
        ...
        try {
            const { location, plan, contract, formData } = {
                location: token_service.state.location,
                plan: token_service.state.selectedPlan,
                contract: token_service.state.selectedContract,
                formData: token_service.state.formData,
            };
    ...
    clearSessionStorage() {
        ...
        try {
            cart_service.cartService.clearCartSession();
            product_service.productService.clearProductSession();
            shipping_service.shippingService.clearShippingData();
            payment_service.paymentService.clearPaymentData();
            plansService.clearPlan();
            token_service.flowActions.clearToken();
        }
        catch (e) {
    ...
    renderSuccess() {
        ...
        return (index.h("div", { class: "step-confirmation__result step-confirmation__result--success" }, index.h("div", { class: "step-confirmation__icon step-confirmation__icon--success" }, index.h("svg", { width: "49", height: "48", viewBox: "0 0 49 48", fill: "none", xmlns: "http://www.w3.org/2000/svg" }, index.h("path", { d: "M24.4237 2C12.0492 2 2 11.8441 2 23.9661C2 36.0881 12.0492 45.9322 24.4237 45.9322C36.7983 45.9322 46.8475 36.0881 46.8475 23.9661", stroke: "#0EA651", "stroke-width": "4", "stroke-miterlimit": "10", "stroke-linecap": "round", "stroke-linejoin": "round" }), index.h("path", { d: "M44.5225 4.19666L21.1852 30.8L12.3818 22.2577", stroke: "#0EA651", "stroke-width": "4", "stroke-miterlimit": "10", "stroke-linecap": "round", "stroke-linejoin": "round" }))), index.h("h2", { class: "step-confirmation__title step-confirmation__title--success" }, SUCCESS_MESSAGES.REQUEST_SUCCESS), index.h("p", { class: "step-confirmation__message" }, SUCCESS_MESSAGES.REQUEST_SUCCESS_SUBTITLE), displayOrderId && (index.h("p", { class: "step-confirmation__order-id" }, "N\u00FAmero de orden: ", displayOrderId)), this.confirmationSent && (index.h("p", { class: "step-confirmation__email-sent" }, "Se ha enviado un correo de confirmaci\u00F3n a tu email."))));
    }
    renderError() {
        return (index.h("div", { class: "step-confirmation__result step-confirmation__result--error" }, index.h("div", { class: "step-confirmation__icon step-confirmation__icon--error" }, index.h("svg", { width: "49", height: "49", viewBox: "0 0 49 49", fill: "none", xmlns: "http://www.w3.org/2000/svg" }, index.h("path", { d: "M24.5 0C38.0096 9.05167e-07 49 10.9899 49 24.5C49 38.0101 38.0096 49 24.5 49C10.9904 49 0 38.0101 0 24.5C0 10.9899 10.99 0 24.5 0ZM24.5 3.78516C13.0775 3.78516 3.78418 13.0774 3.78418 24.5C3.78418 35.9215 13.0774 45.2148 24.5 45.2148C35.9225 45.2148 45.2158 35.922 45.2158 24.5C45.2158 13.078 35.9225 3.78516 24.5 3.78516ZM24.5 31.8809C25.5449 31.8809 26.3915 32.7276 26.3916 33.7725V35.3486C26.3916 36.3936 25.5449 37.2412 24.5 37.2412C23.4551 37.2412 22.6084 36.3936 22.6084 35.3486V33.7725C22.6085 32.7276 23.4551 31.8809 24.5 31.8809ZM24.5 11.7588C25.5449 11.7588 26.3915 12.6055 26.3916 13.6504V27.7725C26.3916 28.8174 25.5449 29.6641 24.5 29.6641C23.4551 29.6641 22.6084 28.8174 22.6084 27.7725V13.6504C22.6085 12.6055 23.4551 11.7588 24.5 11.7588Z", fill: "#B41E13" }))), index.h("h2", { class: "step-confirmation__title step-confirmation__title--error" }, "\u00A1Lo sentimos, ha ocurrido un error en el proceso de solicitud!"), index.h("p", { class: "step-confirmation__message" }, "En este momento estamos presentando inconvenientes en nuestro sistema.", index.h("br", null), "Por favor, int\u00E9ntalo nuevamente.")));
    }
    render() {
        return (index.h(index.Host, { key: 'c9a55cce3d45179061635122b27e6f8383b06c28' }, index.h("div", { key: '6d38397a35a4e1577c6e04b63bf723a833f3c2db', class: "step-confirmation" }, index.h("header", { key: 'fc538465205df4823a6f982ceb4ac077a1e9b7fc', class: "step-confirmation__header" }, index.h("h1", { key: 'e35f03a038c0b20308c23f02f7b9722ef35c088a', class: "step-confirmation__header-title" }, "Confirmaci\u00F3n de Solicitud")), index.h("div", { key: '829ec1fe2b5eb1d9d9ffa5c5b0888c1879f55ab1', class: "step-confirmation__content" }, this.status === 'loading' && this.renderLoading(), this.status === 'success' && this.renderSuccess(), this.status === 'error' && this.renderError()), this.status === 'success' && (index.h("div", { key: 'a9b7287c01ae9c3c8de2d910861366af5eb8dbdc', class: "step-confirmation__actions" }, index.h("button", { key: 'dc2f1935dabdb795e7d89779b956a4d4a3c9d69d', class: "step-confirmation__btn", onClick: this.handleClose }, "Cerrar"))), this.status === 'error' && (index.h("div", { key: 'c563a79251066406241d0995a4e9cdcef1e6fefc', class: "step-confirmation__actions" }, index.h("button", { key: '8d73ee00927aa6a2fc80bb2c0f210babb4a02d4f', class: "step-confirmation__btn step-confirmation__btn--error", onClick: this.handleRetry }, "Volver a intentar"))))));
    }
};
...
const StepContract = class {
    ...
    restorePreviousSelection() {
        ...
        if (token_service.state.selectedContract) {
            this.selectedOption = token_service.state.selectedContract;
            this.activeTab = token_service.state.selectedContract.typeId;
            return;
        }
        ...
        if (typeContractId !== null) {
            ...
            token_service.flowActions.setContract(this.selectedOption);
        }
    }
    ...
    render() {
        ...
        return (index.h(index.Host, { key: '2f9c47b6c246b9276582b054d4acd5b5b93d120b' }, index.h("div", { key: '5b1aeb54cb7a37e15c577c024eac0f55c15304ad', class: "step-contract" }, index.h("header", { key: '8124ea48b7f4b7275506d5eb641fcdce708b6c53', class: "step-contract__header" }, index.h("button", { key: '138d75285752b27f2f9e662d39375ac9bc193050', class: "step-contract__back-link", onClick: this.onBack }, index.h("svg", { key: 'f28c0e80865b2a51790c673fa7942dd897cc2441', viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", "stroke-width": "2" }, index.h("polyline", { key: '9e3dbb79939c6c0bc29d70eb1da75f17ab36abb4', points: "15 18 9 12 15 6" })), index.h("span", { key: 'cec0ed519992936daa1dcf6dcd61bf5da2954ccd' }, "Regresar")), index.h("h1", { key: '2d1c112d4c5e095717c75fab9bf1636220326aa2', class: "step-contract__title" }, "Selecciona un tipo de contrato"), index.h("div", { key: '15bfa6bca6b65dd8c594adeb91bd6d8e8bd24c26', class: "step-contract__divider" })), index.h("div", { key: 'b3a66f3ae45e8083dcbf5582d210860bbc8b4633', class: "step-contract__tabs" }, index.h("button", { key: '8d854b7de5b09a69114ca120df8b63ee207adb94', class: {
                'step-contract__tab': true,
                'step-contract__tab--active': this.activeTab === 1,
            }, onClick: () => this.handleTabChange(1) }, index.h("span", { key: '34a87eebe3a6ea8cbeb78e4a837678ac2ee95688', class: "step-contract__tab-title" }, "Con contrato"), index.h("span", { key: 'bcc3e9b40f4dab3e24ef58e4003dd7faceaf071f', class: "step-contract__tab-subtitle" }, "12 y 24 meses de contrato")), index.h("button", { key: '9864095499d6c08890a6c4515cf12c40a2967d2f', class: {
                'step-contract__tab': true,
                'step-contract__tab--active': this.activeTab === 0,
            }, onClick: () => this.handleTabChange(0) }, index.h("span", { key: '1a4c205a92d04fc91c94b23275be60b44928483e', class: "step-contract__tab-title" }, "Sin contrato"), index.h("span", { key: '8665ee734b582701bd7cf1ee2a9c4f1977c508c5', class: "step-contract__tab-subtitle" }, "Plan mensual con pago por adelantado"))), index.h("div", { key: 'ac54bf034d79f713ea0c41586925d68515e60b55', class: "step-contract__content" }, this.activeTab === 1 && withContract && (index.h("div", { key: '9ee6ddd69a8ff11ad53042d57038869f2e535319', class: "step-contract__options" }, withContract.contract.map((option) => {
        ...
        }))), this.activeTab === 0 && withoutContract && (index.h("div", { key: '4ee8ead6d43fb11e43334d819f3a2472724d17ac', class: "step-contract__options step-contract__options--single" }, withoutContract.contract.map((option) => {
            const totalCost = this.getTotalInstallationCost(option);
            return (index.h("label", { class: {
                    'step-contract__option': true,
        ...
        })))), index.h("div", { key: '1e67eac597810d31136d1ae7fbd373fa52fdeb52', class: "step-contract__actions" }, index.h("button", { key: '3a99d0c5e08de4e256b0e3827ddf79299199347e', class: "step-contract__btn-continue", onClick: this.handleContinue, disabled: !this.selectedOption }, "Continuar")))));
    }
};
...
const StepForm = class {
    ...
    formData = {
        ...
        address: {
            address: token_service.state.location?.address || '',
            city: token_service.state.location?.city || '',
            zipCode: token_service.state.location?.zipCode || '',
        },
    ...
    handleSubmit = (e) => {
        ...
        if (this.validateForm()) {
            token_service.flowActions.setFormData(this.formData);
            this.onNext?.();
        }
    };
    handleBack = () => {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: '052a17c0714cde57a903b9101514376f3b2d3512' }, index.h("div", { key: 'feb6abb25ea09b5e8c1fe352de0bb09cf9844c22', class: "step-form" }, index.h("header", { key: '7368db5e199f05b74ffef1aad5b6af2755c97d4b', class: "step-form__header" }, index.h("button", { key: '3754d869f636700320f388f6454d936b324099bb', type: "button", class: "step-form__back-link", onClick: this.handleBack }, index.h("svg", { key: 'a8d9dd2906d47801c6072c410f17622afe6d75b0', viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", "stroke-width": "2" }, index.h("polyline", { key: 'cde115fd0288b6e23fbaf2029ef59313d9dae969', points: "15 18 9 12 15 6" })), index.h("span", { key: '9ace2f7b98003ea34671e95b66fe901fc0cce127' }, "Regresar")), index.h("h1", { key: '00af145de0cfb2340e1e4fc690f63d60032896e7', class: "step-form__title" }, "Formulario de solicitud de servicio fijo"), index.h("div", { key: 'e4416415e9d1db86242194cd62364a04a92f34c7', class: "step-form__divider" })), this.isMobile && (index.h("div", { key: '0c522c33e3b51c45b621b6f177690ef23a06abb5', class: "step-form__stepper" }, index.h("ui-stepper", { key: '9e3e1d3f51c2f7628d8a5e82bdd62ce36a86e7f7', steps: FORM_STEPS, currentStep: currentStepNumber }))), index.h("form", { key: 'b68783bd19a1ebe5eaf1de5123c1d308bbe27e73', onSubmit: this.handleSubmit }, index.h("p", { key: 'eecb49ec1edb1495f5dc076878ff279949de8017', class: "step-form__instructions" }, "Por favor, ingresa la informaci\u00F3n personal solicitada. Su nombre y apellido deben ser iguales a su identificaci\u00F3n de gobierno."), !this.isMobile && ([
            this.renderIdentificationSection(),
            ...this.renderContactSection(),
        ]), this.isMobile && this.currentSection === 'identification' && (this.renderIdentificationSection()), this.isMobile && this.currentSection === 'contact' && (this.renderContactSection()), index.h("div", { key: 'f02f79569143c69bc842f3704d4f028893b85513', class: "step-form__actions" }, index.h("button", { key: '46f2d379c4ac03714fe207704521ef6699bc8276', type: "submit", class: "step-form__btn-submit", disabled: !this.isCurrentSectionValid() }, "Continuar"))))));
    }
};
...
const StepLocation = class {
    ...
    handleNoConverageWithOptions = () => {
        ...
        if (this.currentCoordinates) {
            ...
            token_service.flowActions.setLocation(claroHogarLocation);
    ...
    handleInfoWindowContinue = () => {
        if (this.locationData && this.locationData.isValid) {
            ...
            token_service.flowActions.setLocation(this.locationData);
            this.onNext?.();
        }
    };
    /**
    ...
    // ------------------------------------------
    render() {
        return (index.h(index.Host, { key: '54a5ff3c6b523ad147b00cf7f09faefce5d22732' }, index.h("div", { key: '2588e009a859cd86e9008ae92f99645b761faf9b', class: "step-location" }, this.isValidating && (index.h("div", { key: '66813c5424a489dc74e715fe92e564ceac3230ed', class: "step-location__validating-overlay" }, index.h("div", { key: '6d328683e83fdc8c1fd568d927f186138c086ce7', class: "step-location__validating-content" }, index.h("div", { key: '724fd2eefa12c952fc7bdd60f6939695334cb2bf', class: "step-location__validating-spinner" }), index.h("p", { key: '5f4c8f9b5a73643485768a4ce5c71f5d4900517d', class: "step-location__validating-text" }, "Validando cobertura...")))), index.h("header", { key: 'ca7063473ec212c22a0a7ce44f1e52b7751437bd', class: "step-location__header" }, index.h("h1", { key: '259333f3ec56803c44af276b1c6a8ce129ffbe09', class: "step-location__title" }, index.h("span", { key: '0fdfaa401f5a4e5fa1aeb8efd20a0f06d91e7d99', class: "step-location__title--highlight" }, "Servicio fijo empresarial"), ' ', "en tu \u00E1rea")), index.h("div", { key: '7ebe784ede8e29933eb6d2a2765a4cb87207a040', class: "step-location__map-container" }, index.h("div", { key: '8e6869f818f168c562aba6eb8bb66c19aa1667a6', class: "step-location__controls" }, index.h("div", { key: '6dfcc99a743689d4579f295914dd3acdde56fbf0', class: "step-location__input-group" }, index.h("span", { key: 'b216766b8fb6a0f5bc8920e17ef62d5a271d7cd3', class: "step-location__input-icon" }, index.h("svg", { key: '967f57b0aea6dce6eaa56ee3027a4978dbfdfb5c', viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", "stroke-width": "2" }, index.h("path", { key: 'e85148c10010909a6671af1efdefb48402a81ef9', d: "M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" }), index.h("circle", { key: '3e5776cd19a8015c80c2f5c12ef4c23bc242a18c', cx: "12", cy: "10", r: "3" }))), index.h("input", { key: 'd5e90625538b6391c191b8b9aa49d8d708d8d067', type: "text", class: "step-location__input", placeholder: "Ingrese su direcci\u00F3n", value: this.address, onInput: this.handleAddressChange, onKeyPress: this.handleKeyPress, ref: (el) => this.addressInput = el }), index.h("button", { key: 'c117e64a77c858d525f221dd17cd5bc5ab443ccd', class: {
                'step-location__btn-validate': true,
                'step-location__btn-validate--loading': this.isValidating,
            }, onClick: this.handleValidate, disabled: this.isValidating || (!this.address.trim() && !this.currentCoordinates) }, this.isValidating ? (index.h("span", { class: "step-location__btn-validate-content" }, index.h("span", { class: "step-location__btn-spinner" }), "Validando...")) : ('Validar'))), index.h("div", { key: '98c6078e1e59c0250ccc27198a9f5da9c2da021b', class: "step-location__location-container" }, index.h("button", { key: 'c6a8d4d359e76cd76603b04d5fbe5d8078347188', class: "step-location__btn-location", onClick: this.handleUseCurrentLocation, disabled: this.isGettingLocation || this.isLoadingMap }, index.h("svg", { key: '9cfad9417379b945beb476a1f136aedfd1672302', class: "step-location__btn-location-icon", viewBox: "0 0 24 24", fill: "currentColor", stroke: "none" }, index.h("path", { key: '8dafe163a8a5ceaf212c9028b10ae819808a705a', d: "M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" })), this.isGettingLocation ? 'Obteniendo ubicación...' : 'Utilizar Ubicación Actual'))), index.h("div", { key: 'ce81d999669f9426744b88a38ad03dac577cef0b', class: "step-location__map" }, this.isLoadingMap && !this.mapError && (index.h("div", { key: '90e445d02b25fbc1a33ed9e0339bc8e4b0b5202e', class: "step-location__map-loading" }, index.h("div", { key: '610fbf68fd393efb694b2b28ca1a60348c8ea6b8', class: "step-location__spinner" }), index.h("p", { key: '6e576450a8025066bb3abc29095e38e1b864016b' }, "Cargando mapa..."))), this.mapError && (index.h("div", { key: '7730c5ff6fd176b918900bc325dfb16e7f80794d', class: "step-location__map-error" }, index.h("p", { key: '5035e4d530ad61b40234c4938e21cc620b97623e' }, this.mapError), !this.googleMapsKey && (index.h("small", { key: '56425e22f8b070245c138d0b1e00bf21ce96464f' }, "Configura la prop google-maps-key en el componente")))), index.h("div", { key: '84d175c59110d08c73a5daedbfbc82ac8608d407', class: "step-location__map-canvas", ref: (el) => this.mapContainer = el, style: { display: this.mapError ? 'none' : 'block' } }))), this.showErrorModal && (index.h("div", { key: '3da65c36d312d597a6f2e16b697f7cf43ed194aa', class: "step-location__modal-backdrop" }, index.h("div", { key: 'fc6c6738e7924d840ce6e78ff67c382f96a9b00d', class: "step-location__modal step-location__modal--error" }, index.h("button", { key: 'd68d55262d914412254a16fa7676972183e6f84d', class: "step-location__modal-close", onClick: () => this.showErrorModal = false }, "\u00D7"), index.h("div", { key: 'cd1acaf7ff3d9c221a6137ea8b4108b57205258b', class: "step-location__modal-error-bar" }, "Error"), index.h("p", { key: 'a3978ce5f0da0fb7ad0fadaeef15ded0fa6e4350', class: "step-location__modal-message" }, this.errorMessage), index.h("button", { key: '1a06f5385b6d04caa1737e21ee34c9c3927820fa', class: "step-location__modal-btn", onClick: () => this.showErrorModal = false }, "Cerrar")))))));
    }
};
...
const StepPlans = class {
    ...
    async initializePlans() {
        ...
        if (storedPlanId > 0) {
            ...
            if (storedPlan) {
                ...
                token_service.flowActions.selectPlan(storedPlan);
            }
        }
    }
    // ------------------------------------------
    ...
    async loadPlans() {
        ...
        this.error = null;
        try {
            const serviceType = token_service.state.location?.serviceType || 'GPON';
            ...
            if (serviceType === 'CLARO HOGAR') {
                ...
                catalogId = product_service.productService.getSubcatalogId();
    ...
    handleSelectPlan = async (plan) => {
        ...
        try {
            ...
            token_service.flowActions.selectPlan(plan);
    ...
    render() {
        ...
        return (index.h(index.Host, { key: 'ecf1db1ef3f9e81483495c3b9586de14f222d94a' }, index.h("div", { key: '560163fc6f813e3147089c3bdfd288f731ab14c7', class: "step-plans" }, index.h("header", { key: '1384894370294a3da5ff3acac72ec92f852f318c', class: "step-plans__header" }, index.h("button", { key: '247577020dd526f44fd70333500847d336dc9345', class: "step-plans__back-link", onClick: this.onBack }, index.h("svg", { key: '0cea3cb99542bd6e2de88b42c0b5addeb1399110', viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", "stroke-width": "2" }, index.h("polyline", { key: '0b8e1b8328600521193fb20d77a81c85a3aba7ee', points: "15 18 9 12 15 6" })), index.h("span", { key: '0aa9cf6a73d66fd469ce5e29d0fcf38cff649a6d' }, "Regresar")), index.h("h1", { key: 'd6a5dcc67d70e97944649e7ad420aedec00cf3f2', class: "step-plans__title" }, "Elige tu plan", serviceTypeLabel), index.h("div", { key: 'd3e74037bc3014ed6b86c266a203bdf17e2a127a', class: "step-plans__divider" })), this.isLoading && (index.h("div", { key: 'f8edc947d6635d4af2e743da6a7c963edd187b1e', class: "step-plans__loading" }, index.h("div", { key: '5997bcd4384af0b61953c8fd9332e99d884f4687', class: "step-plans__spinner" }), index.h("p", { key: '779b7f43edd0df3f701742469c6ba837072cac5d' }, "Cargando planes..."))), this.error && (index.h("div", { key: 'ce1f9b224afaca66c1b88c5607962f78872c391c', class: "step-plans__error" }, index.h("p", { key: '399d3b32d21951f8610efa22de9fe1216b6cda1e' }, this.error), index.h("button", { key: 'c4d64e3869742b9acf717d803c8fa430d0359bf1', onClick: () => this.loadPlans() }, "Reintentar"))), !this.isLoading && !this.error && this.plans.length > 0 && (index.h("div", { key: '2876b099529efb6ca0d43e8c5152d9e126f7616e', class: "step-plans__carousel-container" }, index.h("ui-carousel", { key: '3c26ef5d1ea9803471dfa789583ab1e7a386c3a9', totalItems: this.plans.length, gap: 24, showNavigation: false, showPagination: true, breakpoints: [
                { minWidth: 0, slidesPerView: 1 },
                { minWidth: 500, slidesPerView: 2 },
                { minWidth: 800, slidesPerView: 3 },
                { minWidth: 1100, slidesPerView: 4 },
            ] }, this.plans.map((plan) => this.renderPlanCard(plan))))), !this.isLoading && !this.error && this.plans.length === 0 && (index.h("div", { key: 'c21ad5352e2e72c48c45417119448485fade5805', class: "step-plans__empty" }, index.h("p", { key: 'e01ee7e722da8f17b698a207233ff266655aa71e' }, "No hay planes disponibles para tu \u00E1rea."))), index.h("footer", { key: '88ea2f234ae81f589a8a65b66cc38cb37b702958', class: "step-plans__footer" }, index.h("div", { key: '21ac08d1ab7404d1e1addb7f699eb89398b49ad8', class: "step-plans__footer-left" }, index.h("div", { key: 'de693a69cbb99c1bae6529706ed1ce4f5cdd443d', class: "step-plans__footer-info" }, index.h("div", { key: 'd397c01d84893b21cbd4e880ce22101ee7528367', class: "step-plans__footer-item" }, index.h("span", { key: '9f18194ace853d93f3df83cdb3188b0f68d3fbf4', class: "step-plans__footer-label" }, "Pago mensual"), index.h("span", { key: 'b76fafaf7757922d9ee12ddddfce63d5a8ec3e4b', class: "step-plans__footer-value" }, formatPrice(monthlyPayment))), index.h("div", { key: '862fd05d830e166d049f53ea9be07245f9ea63c7', class: "step-plans__footer-item step-plans__footer-item--separator" }, index.h("span", { key: 'b349c8b75b287a3ed30ac0a26293b744107e30de', class: "step-plans__footer-label" }, "Paga hoy"), index.h("span", { key: '3f195a59422154fb605d8d45b5ddcc47c071762d', class: "step-plans__footer-value step-plans__footer-value--highlight" }, formatPrice(totalToday)))), index.h("p", { key: 'f91b9b15bbba0adadbeed0a6e4c0c23222f4a10f', class: "step-plans__footer-note" }, "Renta mensual aproximada no incluye cargos estatales, federales, ni otros impuestos.")), index.h("button", { key: '0411a1ddc40dfbf1d9f08aa3bb4aad1bdd973e38', class: "step-plans__footer-btn", onClick: this.handleContinue, disabled: !this.selectedPlan || this.isAddingToCart }, this.isAddingToCart ? 'Procesando...' : 'Continuar')))));
    }
};
...
//# sourceMappingURL=fixed-service-flow.step-catalogue.step-confirmation.step-contract.step-form.step-location.step-plans.ui-carousel.ui-stepper.entry.cjs.js.map
