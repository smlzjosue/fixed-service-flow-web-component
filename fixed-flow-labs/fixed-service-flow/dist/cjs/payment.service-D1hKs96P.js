...
class PaymentService {
    ...
    async createOrder(request) {
        ...
        try {
            const response = await token_service.httpService.postFormData('api/Orders/creationOfOrder', formData);
    ...
    async recordPayment(request) {
        ...
        formData.append('deposit', request.deposit);
        try {
            const response = await token_service.httpService.postFormData('api/Payment/record', formData);
            return response;
        }
        catch (error) {
    ...
    async recordPaymentError(ban, subscriber, description, operationId, deposit, cardType, cardNumber, amount) {
        ...
        try {
            return await token_service.httpService.postFormData('api/Payment/error', formData);
        }
        catch (error) {
    ...
    async getPaymentIframe(request) {
        try {
            const response = await token_service.httpService.post('api/Payment/getPaymentIframe', request);
            return response;
        }
        catch (error) {
    ...
    // ------------------------------------------
    storeOrderBan(ban) {
        shipping_service.storageUtils.set(STORAGE_KEYS.ORDER_BAN, ban);
    }
    getOrderBan() {
        return shipping_service.storageUtils.get(STORAGE_KEYS.ORDER_BAN) || '';
    }
    storePaymentResult(result) {
        shipping_service.storageUtils.setJSON(STORAGE_KEYS.PAYMENT_RESULT, result);
    }
    getPaymentResult() {
        return shipping_service.storageUtils.getJSON(STORAGE_KEYS.PAYMENT_RESULT);
    }
    clearPaymentData() {
        shipping_service.storageUtils.remove(STORAGE_KEYS.ORDER_BAN);
        shipping_service.storageUtils.remove(STORAGE_KEYS.PAYMENT_RESULT);
    }
    // ------------------------------------------
...
//# sourceMappingURL=payment.service-D1hKs96P.js.map
