...
// Puerto Rico Zip Codes with municipalities
export interface ZipCodeEntry {
  label: string; // Municipality name
...
const PR_ZIP_CODES: ZipCodeEntry[] = [
...
export class ShippingService {
  ...
  async createAddress(address: ShippingAddress, flowId: string = '5'): Promise<ShippingCreateResponse> {
    ...
    try {
      const response = await this.http.postFormData<ShippingCreateResponse>('api/Address/create', formData);
  ...
  async getDeliveryMethods(): Promise<DeliveryMethodsResponse> {
    ...
    try {
      const response = await this.http.postFormData<DeliveryMethodsResponse>('api/Address/getDeliveryMethods', formData);
      return response;
    } catch (error) {
  ...
  getStoredShippingAddress(): ShippingAddress | null {
    return storageUtils.getJSON<ShippingAddress>(STORAGE_KEYS.SHIPPING_ADDRESS);
  }
  ...
  getStoredDeliveryMethod(): DeliveryMethod | null {
    return storageUtils.getJSON<DeliveryMethod>(STORAGE_KEYS.DELIVERY_METHOD);
  }
