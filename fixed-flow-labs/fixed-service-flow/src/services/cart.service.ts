...
class CartService {
   ...
   */
  async getCart(promoCode: string = ''): Promise<CartResponse> {
    try {
      ...
      const response = await httpService.post<CartResponse>('api/Card/getCart', request);
   ...
   */
  async addToCart(
    product: ProductDetail | CatalogueProduct,
    installments: number,
    quantity: number = 1,
    parentCartId?: number,
    ...
    try {
      ...
      const request: AddToCartRequest = {
        ...
        decDeposit: 0,
        decDownPayment: (product as ProductDetail).decDownPayment || 0,
   ...
   */
  async deleteItem(cartId: number, productId: number): Promise<{ hasError: boolean; message?: string }> {
    try {
      await tokenService.ensureToken();
      ...
      const response = await httpService.post<{ hasError: boolean; message?: string }>(
        'api/Card/deleteItem',
        request
      );
   ...
   */
  async clearCart(): Promise<{ hasError: boolean; message?: string }> {
    try {
   ...
   */
  private storeCartInSession(cart: CartResponse): void {
    try {
